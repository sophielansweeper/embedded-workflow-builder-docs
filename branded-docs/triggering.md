...
Your Workflows URLs will each be unique and will look similar to `https://hooks.Lansweeper.com/trigger/SW5zEXAMPLE1234567890`
...
```bash
curl \
  'https://hooks.Lansweeper.com/trigger/SW5zEXAMPLE1234567890==/my/custom/path?param-one=ParamValueOne&param-two=ParamValueTwo' \
...
```bash
curl 'https://hooks.Lansweeper.com/trigger/SW5zEXAMPLE1234567890' \
  --request POST \
...
curl \
  --data '{}' \
  --header "Content-Type: application/json" \
  'https://hooks.Lansweeper.com/trigger/SW5zEXAMPLE1234567890'
...
1. To provide a consistent HTTP status code for all asynchronous webhook requests, configure the default response code in the Flow Builder by clicking on the trigger at the top of the Workflow editor and opening the **Configure** drawer on the right side of the screen.
